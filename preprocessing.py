...
# Standard size (pixels) that every processed form is resized to, so that
# the field-region coordinates in templates_config.py (given as 0-1 fractions)
# map consistently onto every processed image, regardless of the original
# scan resolution.
STANDARD_WIDTH = 1700
STANDARD_HEIGHT = 2200  # roughly A4 at ~200 DPI, portrait


def load_image(path):
    """Load JPG/JPEG/PNG directly, or the first page of a PDF via PyMuPDF."""
...
def deskew(image):
    """Fine-grained deskew (small angle) using the minimum-area rectangle of
    dark (ink/text) pixels."""
    ...
    if angle < -45:
        ...
        angle = -angle
    # Only correct small skews; large rotations are handled by auto_rotate()
    if abs(angle) > 15:
        return image
...
def perspective_correct(image):
    ...
    small = cv2.resize(image, (int(image.shape[1] / ratio), 1000))
    ...
    for c in contours:
        ...
        if len(approx) == 4 and cv2.contourArea(approx) > 0.4 * small.shape[0] * small.shape[1]:
            doc_contour = approx
            break

    if doc_contour is None:
        return orig  # nothing confident found; skip perspective warp
...
def correct_brightness_contrast(image):
    """Auto contrast stretch using CLAHE on the L channel (LAB colour space)."""
...
def adaptive_threshold(gray_image):
    return cv2.adaptiveThreshold(
        gray_image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 25, 15
    )
...
def align_to_reference(image, reference_path):
    ...
    good = matches[: max(20, int(len(matches) * 0.15))]
...
def preprocess_pipeline(input_path, reference_template_path=None, save_path=None):

    image, _warnings = preprocess_pipeline_verbose(input_path, reference_template_path, save_path)
    return image
...
def preprocess_pipeline_verbose(input_path, reference_template_path=None, save_path=None):
    ...
    if reference_template_path:
        ...
        # align_to_reference silently returns the input unchanged when it
        # can't find enough good ORB matches; detect that by comparing content.
